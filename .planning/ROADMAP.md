...
Two phases mapping directly to the two project tasks. Phase 1 delivers a fully working end-to-end chat application: OpenRouter backend wired to GPT-4o mini, bubble UI, per-session conversation memory. Phase 2 adds model switching on top of the working chat, letting the user choose between available OpenRouter models from the UI.
...
- [ ] **Phase 1: Working Chat** - OpenRouter backend + bubble chat UI with per-session conversation memory
- [ ] **Phase 2: Choose Your Model** - Model selector in the UI so the user can switch LLMs mid-session

## Phase Details

### Phase 1: Working Chat
**Goal**: Users can open the app, send a message, get a real answer from GPT-4o mini via OpenRouter, and continue the conversation across multiple turns in the same browser session
**Mode:** mvp
**Depends on**: Nothing (first phase)
**Requirements**: BACK-01, BACK-02, BACK-03, BACK-04, BACK-05, UI-01, UI-02, UI-03, UI-04, CONV-01, CONV-02

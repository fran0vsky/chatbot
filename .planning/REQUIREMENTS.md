...
- [x] **PROD-05**: There is exactly one documented, working frontend-serving path (the Docker-baked frontend behind Caddy); the vestigial GCS frontend deploy job is removed or repurposed, and the deployment runbooks describe the real Caddy/baked-frontend architecture
